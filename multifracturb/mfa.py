...
def measure(data, boxSize):
    ...
    if(boxSize == 1):
        ...
        mu = np.zeros((linear_boxes,linear_boxes,linear_boxes))
        for i in range(linear_boxes):
            for j in range(linear_boxes):
                for k in range(linear_boxes):
                    mu[i,j,k] = np.sum(S[i*r:(i+1)*r, j*r:(j+1)*r, k*r:(k+1)*r])
        return mu/Norm
...
def compute_measures(data, save=False, save_in_file="mu.h5"):
    ...
    if(Nx==Ny and Ny == Nz):
        ...
        print("ERROR: The array should have equal elements along each axis.")
        return 1
...
def plots_for_scaline_range(mf_data, data_of, plots_dir="."):
    """
...
def plotsGrid_for_scaline_range(mf_data, data_of, plots_dir="."):
    """
    Plots the multifractal data with log(l) to determine the linear scaling range, manually.
    ...
    grid_size = int(np.sqrt(len(moments)))
    width = 7 * grid_size
    height = width/1.4
    fig, ax = plt.subplots(grid_size, grid_size, True, True, figsize=(width, height))
    i = -1 
    for iq, q in enumerate(moments):
        if(q!=0):
            numerator = mf_data[iq+1, 1:]
